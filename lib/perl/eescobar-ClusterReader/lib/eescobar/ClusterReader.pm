...
eescobar::ClusterReader - Perl extension for ESTs contigs.

=cut

=head1 SYNOPSIS

  This program's purpose is to read cluster.seq.tab.

# Change this line to show the folder where you store ClusterReader.pm
  use constant  PERLELIB => $ENV{"PERLELIB"}  ;
  use lib PERLELIB;
  use eescobar::ClusterReader;
...
#methods
  fetchCLUSeqTab  (I<$fil,$ctg>)  return  string representing cluster.seq.tab sequence
  fetchCLUSeqTabAt (I<$fil,$ctg,$pos>)  return  string representing cluster.seq.tab base pair
...
=cut
package eescobar::ClusterReader;
...
our @EXPORT     = qw  (fetchCLUSeqTab,fetchCLUSeqTabAt)  ;
...
Stub documentation for eescobar::ClusterReader, created by h2xs.

ClusterReader.pm   a program that calls celutil.

# Change this line to show the folder where you store System.pm
use constant  PERLELIB => "/opt/Suite/ESTsSuite/trunk/lib/perl/eescobar-ClusterReader/lib"  ;
use lib PERLELIB;
use eescobar::ClusterReader;

cluster.seq.tab reader :  fetchCLUSeqTab($fil,$ctg), fetchCLUSeqTabAt($fil,$ctg,$pos)
...
=head2 fetchCLUSeqTab (I<$fil,$ctg>)

This method receives a cluster filehandle to verify its sequence.

=over 4

=item @precond

fasta2line.pl must be used to create a cluster.seq.tab file

=item @param  I<$fil>

a string representing a cluster.seq.tab filehandle

=item @return I<$ctg>

a string representing a cluster
...
# file opening for read
  open  ( CLUFILE, $fil )    or     die ( "Cannot open  file : $fil\n" );
#   print ( STDERR "Reading file (".$fil.")\n" );
  @info       = <CLUFILE>;
  chomp ( @info );
# close the CLUFILE file
  close ( CLUFILE )          or     die ( "Cannot close file : $fil\n" );
# fetching read
  ($lin)      = $ctg =~ /(Contig[0-9]+)$/;
  @info       = grep( /^$lin/,@info );
  $lin        = $info[@info-1];
...
=head2  fetchCLUSeqTabAt (I<$fil,$ctg,$pos>)
...
#testing methods
# Change this line to show the folder where you store System.pm
  use constant  PERLELIB => "/home/eescobar/PerlCode/eescobar-ClusterReader/lib"  ;
...
#tests
  ...
  print ( &fetchCLUSeqTab($mycelfile)."\n" );
...
B<ClusterReader.pm @ Quique's web page> for more information http://adn.bioinfo.uqam.ca/~escd07097301/

=begin html

<a href="http://adn.bioinfo.uqam.ca/~escd07097301/mycode/PerlCode/eescobar/ClusterReader.html">ClusterReader.pm</a>
